...
export function previewReport(fixture: Fixture, selection: string, locale: Locale = 'en') {
  const unavailablePeriod = locale === 'fr' ? 'Période indisponible' : 'Period unavailable'
  const unavailableData = locale === 'fr' ? 'Donnée indisponible' : 'Data unavailable'
  ...
  if (!period) return { title: `${unavailablePeriod}: ${periodId}`, errors: [`${unavailablePeriod}: ${periodId}`], rows: [], canInsert: false, expected: negative?.expected }
  ...
  const errors = rows.filter(row => row.value == null || !Number.isFinite(row.value)).map(row => `${unavailableData}: ${row.label}`)
